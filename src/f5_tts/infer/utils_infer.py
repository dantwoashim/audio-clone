    ...
    if vocoder_name == "vocos":
        ...
        if is_local:
            ...
            try:
                ...
                print("Load Vocos from local Hugging Face cache")
            except Exception:
...
def infer_batch_process(
    ...
    if streaming:
        ...
        run_parallel = str(device).startswith("cuda") and len(gen_text_batches) > 1

        if run_parallel:
            with ThreadPoolExecutor() as executor:
                futures = [executor.submit(infer_single_process, gen_text) for gen_text in gen_text_batches]
                results = (future.result() for future in (progress.tqdm(futures) if progress is not None else futures))
